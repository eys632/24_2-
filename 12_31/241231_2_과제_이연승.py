...
# HTML 파일을 Markdown 형식에서 변환한 텍스트로 가정
html_content = """
# 이력서
...
splitter = HTMLHeaderTextSplitter(
    headers_to_split_on=["h1", "h2", "h3"]  # 분석하고자 하는 헤더 태그
)
...
for doc in documents:
    print(f"헤더: {doc.metadata.get('header', '없음')}")
    print(f"내용: {doc.page_content}\n")
    print("=" * 20)

# 결과를 파일에 저장 (선택 사항)
output_file = "split_headers_output.txt"
with open(output_file, "w", encoding="utf-8") as file:
    for doc in documents:
        file.write(f"헤더: {doc.metadata.get('header', '없음')}\n")
        file.write(f"내용: {doc.page_content}\n\n")

print(f"분할 결과가 {output_file}에 저장되었습니다.")
